...
const CardOne = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data, error } = await getRequest("/provider/card/booking-count");
      if (data) {
  ...
  return (
    <div className="rounded-sm border border-stroke bg-white py-6 px-8 shadow-default">
      <div className="flex h-12 w-12 items-center justify-center rounded-full bg-meta-2">
        <TbBrandBooking size={22} className="text-primary" />
      </div>

      <div className="mt-6 flex items-end justify-between">
        <div className="w-full flex justify-between items-center">
          <span className="block text-sm font-medium">Total Bookings</span>
